...
if(isset($_POST['save'])){
	...
	foreach($amount_arr as $index => $amount){
		...
		mysql_query(" Update `orders_tov` set `amount` = '$amount' where `id` = '$id' ") or die(mysql_error());
	}
...
<script>
	$(function(){
		$('#order-save').click(function(){
            ...
            console.log(amount_arr, tov_arr);
			
			console.log(location.pathname);
			$.post(location.pathname, {save:1, 'amount[]':amount_arr, 'tov[]':tov_arr}, function(data){
				console.log(data);
